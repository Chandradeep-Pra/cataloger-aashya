...
export default function FlipText({
  ...
  // Explicitly type refs: array of span elements (or null before mount)
  ...
  useEffect(() => {
    ...
    // Filter out nulls before passing to GSAP
    ...
    gsap.set(topEls, { opacity: 0, rotationX: 90 });
    gsap.set(bottomEls, { opacity: 0, y: 10 });
    ...
    // Step 3: Push wrong letters up, reveal correct text
    tl.to(
      topEls,
      {
        y: -40,
    ...
    tl.to(
      ...
      "<" // start same time as top moving out
    );
  }, [text, flippedText]);
    ...
    >
      <span className="text-3xl md:text-7xl font-krona-one md:pr-0 pr-12 md:pl-10 md:pt-8 flex gap-1 relative">
        {text.split("").map((char, i) => (
          <span
            key={i}
            className="relative inline-block overflow-hidden h-[2.5rem] w-[1.7rem] md:w-[4rem] md:h-[10rem]"
          >
            {/* Top: Wrong Text */}
            <span
              ref={(el) => {
